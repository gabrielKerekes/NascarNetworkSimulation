...
{       
    ...
    for (int testNumber = 0; testNumber < numOfTests; testNumber++)
    {
        ...
        averages[testNumber] = sum/numOfRuns;
        sum = 0;
        for (int runNumber = 0; runNumber < numOfRuns; runNumber++)
        {
...
int main(int argc, char *argv[]) {
    ...
    for (int runNumber = 0; runNumber < numOfRuns; runNumber++) {
        ...
        for (int i = 0; i < numOfTests; i++) {
            std::cout << runsThroughputs[runNumber][i] << "Mbps - " << runsPacketsReceiveds[runNumber][i] << "/" << runsPacketsReceiveds[runNumber][i] << "\n";
        }
    }
    
    DeviationStuff(runsThroughputs, numOfRuns, numOfTests);
